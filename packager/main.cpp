...
int main(int _argc, char** _argv) {
  ...
  for (const path &input : opt.inputs_) {
    ...
    if (cgltf_parse_file(&options, (char*)input.u8string().data(), &rawdata) != cgltf_result_success) {
    ...
    if (cgltf_load_buffers(&options, data.get(), (char*)input.u8string().c_str()) != cgltf_result_success) {
